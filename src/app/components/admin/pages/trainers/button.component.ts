...
export class ButtonViewComponent implements ViewCell, OnInit {
  ...
  unlock() {
      ...
      console.log("amine",this.rowData.trainer_id)
      this.ngxBootstrapConfirmService.confirm(options).then((res: boolean) => {
        if (res) {

              this.service.activeAccount(this.rowData.trainer_id).subscribe(
                (res) => {
  ...
  lock() {
    ...
    this.ngxBootstrapConfirmService.confirm(options).then((res: boolean) => {
      if (res) {
            ...
            this.service.DesacitveAccount(this.rowData.trainer_id).subscribe(
              (res) => {
                
                console.log("res : ",res);

                this.save.emit(this.rowData);
                this.toastrService.error('Ooh!', 'Trainer account was locked !');
